...
const UserHome = () => {
  ...
  const { adminViewUserId } = useParams(); // 1. Grab ID from URL
  const loggedInUser = JSON.parse(localStorage.getItem("user"));
  
  // 2. The "Switch": Use URL ID if Admin is viewing, otherwise use own ID
  const effectiveUserId = adminViewUserId || loggedInUser?.id;
  
  const isAdminMode = Boolean(adminViewUserId); // 3. Optional: A flag to indicate if we're in Admin view mode (can be used for conditional UI)
...
  // --- LOGIC ADDED: Determine if the selected week is current (0) or previous (1) ---
...
 useEffect(() => {
    const fetchData = async () => {
      // CHANGE 1: Use effectiveUserId instead of user.id
      // This ensures we fetch data for the user the admin clicked on
      ...
      try {
        ...
        // CHANGE 2: Fetch logs for the effectiveUserId
    ...
    fetchData();
    // CHANGE 3: Add effectiveUserId to the dependency array
...
        formatted = `${Math.floor(num).toString().padStart(2, '0')}:00`;
      }
    }
    handleInputChange(date, 'hours_worked', formatted);
  };
    ...
    setLogData((prev) => {
      ...
      if (field === "day_type") {
        if (value !== "Working") {
          ...
          // RESET FOR WORKING (The fix for your issue)
...
    if (!timeStr) return 0;
    if (typeof timeStr === 'number') return timeStr; // Handle cases where it's already a number
...
      return {
        work_date: day.dbDate,
        day_type: data?.day_type || "Working",
        project_name: data?.project_name || "",
        module_name: data?.module_name || "",
        task_description: data?.task_description || "",
        hours_worked: toDecimal(data?.hours_worked),
        is_wfh: data?.is_wfh ? 1 : 0
      };
    });

    const incomplete = weeklyLogs.some(log => !log.project_name || !log.hours_worked);
    if (incomplete) {
      toast.error("Please fill all weekdays (Mon-Fri) before submitting!");
      return;
    }
...
        body: JSON.stringify({ userId: effectiveUserId, // Changed from user.id to effectiveUserId, 
              logs: weeklyLogs })
      });
...
const saveDay = async (day) => {
    ...
    const hasProject = !!data?.project_name && data?.project_name !== "" && data?.project_name !== "N/A";
    const hasHours = !!data?.hours_worked && data?.hours_worked !== "00:00";
    
    // Check if description is filled AND not just the leftover "Leave/Holiday" text
    const hasValidDesc = !!data?.task_description?.trim() && 
                         data?.task_description !== "Leave" && 
                         data?.task_description !== "Holiday";

    if (isWorking) {
      if (!hasProject || !hasValidDesc || !hasHours) {
        toast.error("Please fill all working details (Project, Task description, and Hours)!");
        return;
      }
      if (data.project_name === "Others" && !data.module_name?.trim()) {
        toast.error("Module Name is compulsory for 'Others' project");
        return;
      }
    }

    // ... rest of your existing fetch code remains the same ...
    try {
      const response = await fetch("http://localhost:5000/api/tasks/save-day", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId: effectiveUserId, // Changed from user.id to effectiveUserId
          day_type: dayType,
          project_name: data.project_name,
          module_name: data.module_name || "",
          work_date: day.dbDate,
...
          Hello, {user?.name || 'User'}! Welcome to Suchana Enterprises
        </h1>
...
{isAdminMode && (
  <div className="bg-amber-100 border-l-4 border-amber-500 p-4 mb-4 flex justify-between items-center">
    <div>
      <p className="text-amber-700 font-bold">ADMIN MODE</p>
      <p className="text-xs text-amber-600">Currently editing User ID: {adminViewUserId}</p>
    </div>
    <button 
...
        {/* Banner for Archived Weeks */}
        {!isWeekEditable && (
          <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-center gap-3 text-amber-700 shadow-sm">
            <Eye size={18} />
            <span className="text-sm font-bold">Viewing Archive: Editing is only allowed for the current and previous week.</span>
          </div>
        )}

        <div className="mb-4 bg-gray-900 text-white p-4 rounded-2xl shadow-sm">
           <h2 className="text-sm font-black uppercase tracking-[0.2em]">Enter Weekly Task</h2>
        </div>
...
                           <span className="text-red-500 font-black uppercase tracking-widest text-xs">Week Off</span>
                        </td>
                      ) : (
                        <>
                          <td className="px-6 py-4">
                            <select 
                              className="bg-transparent border-none text-sm font-bold text-blue-600"
                              value={logData[day.dbDate]?.day_type || "Working"}
                              disabled={isDisabled}
                              onChange={(e) => handleInputChange(day.dbDate, 'day_type', e.target.value)}
                            >
...
                              {dbProjects.map(p => <option key={p.id} value={p.project_name}>{p.project_name}</option>)}
...
                          {isWeekEditable && !day.isWeekOff && (
                            (!isSaved || isEditing) ? (
                              <button onClick={() => saveDay(day)} disabled={day.isFuture} className="bg-emerald-500 hover:bg-emerald-600 text-white px-2 py-1 rounded-md text-[10px] font-bold">
                                <SaveIcon size={12}/> {isEditing ? "Update" : "Save"}
                              </button>
                            ) : (
...
                          )}
                          {!day.isWeekOff && (
                            <button onClick={() => setViewData(day)} className="p-1.5 text-gray-500 hover:bg-gray-100 rounded-md"><Eye size={16}/></button>
                          )}
                        </div>
                      </td>
                    </tr>
...
              <button 
                onClick={submitWeeklySheet} 
                className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-2xl font-black text-sm shadow-lg transition-all flex items-center gap-2"
              >
