...
const UserHome = () => {
  ...
  if (isAdminMode && adminViewUserId) {
    fetch(`${API_BASE_URL}/admin/users?limit=9999`)
      .then(res => res.json())
      .then(data => {
...
const response = await fetch(`${API_BASE_URL}/tasks/get-logs/${effectiveUserId}`);
  ...
  return (
    <>
      ...
      {isLoading && (
        <div style={{
          ...
          <div className="text-center animate-pulse">
            <p className={`text-sm font-black uppercase tracking-widest ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}>
              Please wait...
            </p>
            <p className={`text-[10px] font-medium mt-1 ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>
              Fetching your timesheet data
            </p>
          </div>
        </div>
      )}
      ...
      <div className={`
        min-h-screen bg-gray-50 dark:bg-slate-950 font-sans text-gray-900 dark:text-slate-100 transition-all duration-300
        ${isLoading ? 'blur-sm pointer-events-none' : 'blur-0'}
      `}>
      {/* VIEW MODAL */}
      {viewData && (
...
>
  ...
  <textarea
    ...
    rows="1"
    className={` text-sm px-2 py-1 transition-all shadow-sm ${
      isDisabled || isDayOff 
        ? "resize-none border-transparent w-full bg-transparent dark:text-white" 
        : "resize min-w-[120px] min-h-[38px] max-w-[400px] max-h-[200px] bg-white dark:bg-slate-800 border-gray-300 dark:border-slate-600 text-gray-900 dark:text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 z-10 relative"
    }`}
